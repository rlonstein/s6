...
static void logdir_init (logdir_t *ap, unsigned long s, unsigned long n, unsigned long tolerance, uint64 maxdirsize, struct taia const *retrytto, char const *processor, char const *name, unsigned int index)
{
  struct stat st ;
...
static int script_init (genalloc *sc, char const *const *argv)
{
  ...
  unsigned long cur_s = 99999 ;
  unsigned long cur_n = 10 ;
  unsigned long cur_tolerance = 2000 ;
  ...
  for (; *argv ; argv++)
  {
    switch (**argv)
    {
      ...
      case 'n' :
      {
        if (!ulong0_scan(*argv + 1, &cur_n)) goto fail ;
        if (cur_n < 2) cur_n = 2 ;
        break ;
      }
      case 's' :
      {
        if (!ulong0_scan(*argv + 1, &cur_s)) goto fail ;
      ...
      case 'l' :
      {
        if (!ulong0_scan(*argv + 1, &cur_tolerance)) goto fail ;
        if ((cur_tolerance << 1) > cur_s)
      ...
      {
        unsigned long t ;
        if (!ulong0_scan(*argv + 1, &t)) goto fail ;
        if (!taia_from_millisecs(&cur_retrytto, (long)t)) return (errno = EINVAL, 0) ;
        break ;
      }
      case 'E' :
      {
